...
def get_moves(tab=tab):
    moves=[]
    for i in range(9):
        if tab[i]==0:moves.append(i)
    return moves

def game_end(tab=tab):
    cont=0
    for i in tab:
        if i==1 or i==2:cont+=1
    if cont==9:return 1
    else:return 0
...
def draw():
    if not win() and not lose() and game_end():return 1
       
tab_copy=[0,0,0,0,0,0,0,0,0]

def copy_tab(tab=tab,tab_copy=tab_copy):
    for i in range(9):tab_copy[i]=tab[i]
copy_tab()

def copy(tab_copy=tab_copy,tab=tab):
    for i in range(9):tab[i]=tab_copy[i]

def count_tab(tab=tab):
    cont=0
    for i in range(9):
        if tab [i]==1 or tab[i]==2:cont+=1
    return cont
    ...
    if turn=="min":
       if not win() and not lose() and not game_end() and depth!=0:
           ...
           for move in moves:
                 ...
                 tab[move]=0 ; update()
                 if result<score:score=result;best_move=move
           tab[best_move]=2 ; update()
           
    
    if turn=="max":turn="min"
    elif turn=="min":turn="max"
